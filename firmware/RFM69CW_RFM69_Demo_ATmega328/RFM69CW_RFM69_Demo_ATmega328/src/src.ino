 ...
 * h    - help
 * i    - set node
 * j    - send local temperatur
 * k *@ - KAKU command (433 MHz)
 * l    - show led activity
 * m    - dump registers
 * n    - set encription key
 * o    - change freq offset
 * p    - promiscuite mode
 * q    - quiet mode
 * r *@ - replay log markers (JeeNode sample)
 * r    - ACK reply (overriden command)
 * s    - send packet without ACK required
 * t    - broadcast max size packet
 * u    - upload firmware FOTA
 * v    - display version and configuration
 * w *@ - wipe memory
 * w    - predefined ACK reply (overriden command)
 * x    - reporting format
 * y    - enable signal strength mode
 * z *  - total power down
 */

//[RF69demo.13] E i5 g250 @ 868 MHz p1 j1
...
"  <nn> j     - sends local temperature to node <nn>\n"
"             - <nn>=256 displays temp in 1min time frames\n"
"             - <nn>=257 stop displaying temp 1min time frames\n"
...
static void handleInput (char c) {
  ...
  if (c == '>') {
    ...
    */
    ...
    case 'r': // send ACK reply data packet to node <nn>
    case 'w': //preddefined ACK reply data packet to node <nn>
      cmd = c;
      sendLen = top;
      dest = value;
      break;
...
void loop() {
  ...
  if (trace_mode == 0) {
    ...
    if ((millis() - lastMillis>=60000) && config.show_temperature)  {
        lastMillis=millis();
        byte temperatur = radio.readTemperature(-1);
        ...
        if (radio.ACKRequested() && (config.collect_mode == 0)) {
          if (radio.TARGETID == config.nodeId) {          
            if (ackLen==0) {
              ...
              showString(PSTR(" -> ack with buffer "));            
              for (byte i = 0; i < ackLen; ++i) {
                if (!config.hex_output)
                  printOneChar(' ');
                showByte(stack[i]);
              }              
              showString(PSTR("\n"));
              radio.sendACK(stack,ackLen);              
            }
            ackLen = 0;
          } else {
    ...
    if (cmd && radio.canSend()) {
      ...
      showString(PSTR(" b "));
      switch (cmd) {
        case 'a': //"  ...,<nn> a - send data packet to node <nn>, request ack\n"
        ...
        case 's': //"  ...,<nn> s - send data packet to node <nn>, no ack\n"
          radio.send(dest, stack, sendLen);
          showString(PSTR(" ACK not required"));
          break;
          
        case 'r': //"  ...,<nn> r - send ACK reply data packet to node <nn>  (nn is ignored)
          radio.sendACK(stack, sendLen);
          showString(PSTR(" ACK with buffer sent"));
          break;

        case 'w': //preddefined ACK reply data packet to node <nn>\n"
          showString(PSTR(" buffer for next ACK packet Set"));
          ackLen=sendLen;
          break;
        default:
          SerialX.print("  ");SerialX.print(cmd);
          showString(PSTR(" wrong command"));
      }
      showString(PSTR("\n"));
      cmd = 0;
      sendLen = 0;
